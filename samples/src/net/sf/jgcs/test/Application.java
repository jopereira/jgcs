...
public class Application implements MessageListener,
	...
	public Application(ProtocolFactory pf, GroupConfiguration g, Service service) throws JGCSException {
		...
		////
		/// Using the configuration object provided by a the configuration process and
		/// the previously created protocol, instances of data and control sessions
		/// can now be created. A data session will be used to send and receive
		/// messages. The control session will be used to join the group and 
		/// receive notifications concerning the other elements of the group.
		////
	...
	////
	/// This method will run after the creation of the class.
	/// At this point, all the necessary objects were already
	/// retrieved from the lookup service.
	/// The application joins the group, sends some messages
	/// and finally leaves the group.
	////
	public void run() {
		try {
			control.join();
			for (int i = 0; i < NUM_MESSAGES; i++) {
				Thread.sleep(1000);
				////
				/// A new message object must be created using 
				/// the data session. 
				////
				Message message = data.createMessage();
				message.setPayload("hello world!".getBytes());
				////
				/// The message is sent to the group using the service
				/// previously retrieved from the lookup service.
				////
	...
	////
	/// This method represents the message listener. Every time that a message
	/// is sent to the group, it is received in this callback by all elements of the group.
	/// The application can return an object to identify this particular message in the future,
	/// but this feature is not used at the moment. This feature is discussed in other sample.
	////
	...
	/// It is only used if the membership extentions were implemented and may be used instead
	/// of the previous call backs. The new membership may be retrieved from the membership session.
	////
	...
	////
	/// This call back notifies the application that the group will block and
	/// a new membership will be received. The application must flush any pending
